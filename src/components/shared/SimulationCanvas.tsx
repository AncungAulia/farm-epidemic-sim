  ...
  const render = useCallback(() => {
    ...
    for (let i = 0; i < agents.length; i++) {
      ...
      if (sheet) {
        ...
        let row: number
        if (isIdle) {
          row = direction === DIRECTION.DOWN  ? SPRITE.ROW_IDLE_DOWN
              : direction === DIRECTION.UP    ? SPRITE.ROW_IDLE_UP
              : direction === DIRECTION.LEFT  ? SPRITE.ROW_IDLE_LEFT
              : SPRITE.ROW_IDLE_RIGHT
        } else {
          row = direction === DIRECTION.DOWN  ? SPRITE.ROW_WALK_DOWN
              : direction === DIRECTION.UP    ? SPRITE.ROW_WALK_UP
              : direction === DIRECTION.LEFT  ? SPRITE.ROW_WALK_LEFT
        ...
        ctx.drawImage(
          ...
          Math.round(x - frameW / 2), Math.round(y - frameH / 2),
          frameW, frameH,
        )
      } else {
        ctx.fillStyle = color
